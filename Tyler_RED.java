...
@Autonomous(name = "Red Block", group = "Concept")

public class Tyler_RED extends LinearOpMode {
         BNO055IMU imu;
         BNO055IMU imu2;
...
public void forward(int Distance){
      double rpower, lpower, ln, rn, power = 1;
...
if( runtime.seconds() < 2){
    ...
    telemetry.update();
        ...
        double down = Math.abs(Distance)-(Math.abs(robot.frm.getCurrentPosition())+Math.abs(robot.flm.getCurrentPosition()))/2;
    if(Math.abs(down) <301&& Math.abs(down)>10){
        rpower*=(down/300);
        lpower*=(down/300);
    }
    else if(Math.abs(down)<10){
